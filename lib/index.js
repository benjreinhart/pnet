...
void function () {
  var cache$, extend, getParamErrors, normalizeMethod, omit, parseResponseBody, pnet, pnetConfig, pnetDefaults, request, safeExtend, Setlist, SETLIST_METHODS, urlFor;
  ...
  pnet.get = function (method, params, callback) {
    var cache$1, error, url;
    if (null == params)
      params = {};
    method = normalizeMethod(method);
    if (error = null != (cache$1 = getParamErrors(method, params)) ? cache$1[0] : void 0) {
    ...
    if (in$(method, SETLIST_METHODS))
      parsed = function (accum$) {
        ...
        for (var i$ = 0, length$ = parsed.length; i$ < length$; ++i$) {
          show = parsed[i$];
          accum$.push(extend(show, Setlist.parse(show.setlistdata)));
        }
        return accum$;
      }.call(this, []);
    return parsed;
